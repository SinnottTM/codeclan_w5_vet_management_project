...
#CREATE
def save(kaiju):
    sql = "INSERT INTO kaiju( name, dob, type, vet_id, treatment_notes ) VALUES ( %s, %s, %s, %s, %s, %s ) RETURNING *"
    values = [kaiju.name, kaiju.dob, kaiju.kaiju_type, kaiju.vet_assigned, kaiju.treatment_notes]
...
def select_all_kaiju():
    ...
    for row in results:
        kaiju = Kaiju(row['name'], row['dob'], row['type'], row['vet_id'],
                        row['treatment_notes'])
...
def select_single_kaiju(id):
    ...
    if result is not None:
        kaiju = Kaiju(kaiju.name, kaiju.dob, kaiju.kaiju_type, kaiju.vet_assigned, kaiju.treatment_notes)
    return kaiju
